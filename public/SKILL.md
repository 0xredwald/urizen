...
SPY/QQQ — from a non-custodial vault (can-trade, **cannot-withdraw**). This skill exposes the
fund itself: its strategies, its live book, and a way for any agent to copy-trade it or take
one-token exposure via $URI.

- **Homepage:** https://urizen.fund
- **API base:** `https://urizen.fund/api`
- **Manifest:** https://urizen.fund/api/skill
...
curl https://urizen.fund/api/fund/book

# Copy-trade weights
curl https://urizen.fund/api/fund/mirror

# Reads behind the book (momentum / trend / RSI)
curl https://urizen.fund/api/fund/signals

# A ready-to-sign buy of 0.01 ETH of $URI
curl "https://urizen.fund/api/fund/quote?side=buy&amount=0.01&taker=0xYOURWALLET"
```
